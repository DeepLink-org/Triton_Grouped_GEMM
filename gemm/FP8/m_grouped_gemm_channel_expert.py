...
def get_cuda_autotune_config():
    return [
        ...
        triton.Config({ 'BLOCK_N': 256, 'BLOCK_K': 128,}, num_stages=3,
                      num_warps=8),
        triton.Config({ 'BLOCK_N': 128, 'BLOCK_K': 128,}, num_stages=3,
                      num_warps=8),
        triton.Config({'BLOCK_N': 64, 'BLOCK_K': 128,}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_N': 256, 'BLOCK_K': 128,}, num_stages=4,
                      num_warps=4),
        triton.Config({ 'BLOCK_N': 128, 'BLOCK_K': 128,}, num_stages=4,
                      num_warps=4),
        triton.Config({ 'BLOCK_N': 64, 'BLOCK_K': 64,}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_N': 128, 'BLOCK_K': 64,}, num_stages=4,
                      num_warps=4),
        triton.Config({ 'BLOCK_N': 32, 'BLOCK_K': 64,}, num_stages=4,
                      num_warps=4)
    ]
...
def gmm_fp8_act_per_channel_w_per_expert_kernel(
    ...
    for tile_id in tl.range(start_pid, num_tiles, BLOCKS):
        ...
        group, group_pad_off, group_pad_off_nxt = get_group_id(pid_m * BLOCK_M, group_pad_offs, 0, num_groups)
        ...
        for k in tl.range(0, tl.cdiv(K, BLOCK_K)):
            ...
            # load ab
            a = tl.load(a_ptrs, mask=(offs_k[None, :] < K - k * BLOCK_K) & (offs_am[:, None] < token_pad_end), other=0.0)
...
def gmm_fp8_act_per_channel_w_per_expert(A: Tensor,
                     A_scale: Tensor,
                     B: Tensor,
                     B_scale: torch.Tensor,
                     size_per_group: torch.Tensor,
                     dtype_out: torch.dtype = torch.float16) -> Tensor:
    assert A.dim() == 2
    assert B.dim() == 3
    M, K = A.shape
