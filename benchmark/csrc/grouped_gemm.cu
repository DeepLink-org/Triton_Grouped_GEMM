...
using GpuArch = ::cutlass::arch::Sm80;
...
>
RawGemmArguments MakeArgumentsOnDevice(int num_experts, const torch::Device& device, int available_sm_count) {
    TORCH_CHECK(
    ...
    return RawGemmArguments {
      ...
      .threadblock_count = Gemm::sufficient(nullptr, 0, available_sm_count),
    };
}
...
typename Gemm::Arguments MakeArguments(torch::Tensor a,
  ...
  if (batch_sizes.is_cuda()) {
    ...
    >(num_experts, a.device(), available_sm_count);
  } else {
...
torch::Tensor CutlassGroupedGemm(torch::Tensor a,
  ...
  auto arguments = MakeArguments<
    ...
    ElementA, ElementB, ElementC,
    LayoutA, LayoutB, LayoutC
  >(a, b, c, batch_sizes, coord_template, num_experts, available_sm_count);
...
void GroupedGemm(torch::Tensor a,
  ...
  if (trans_a) {
    CutlassGroupedGemm<true, false>(a, b, c, batch_sizes, coord_template, available_sm_count);
    return;
  }
  if (trans_b) {
    CutlassGroupedGemm<false, true>(a, b, c, batch_sizes, coord_template, available_sm_count);
    return;
  }
